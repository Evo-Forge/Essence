...
(function ($) {
	...
	$.fn.rippleEffect = function (options) {
		...
		var startRippleEffect = function startRippleEffect(element, e) {
		    ...
		    endRippleEffect = function endRippleEffect($element) {
			$element.remove();
		};

		$(this).addClass('e-ripple');
		$(this).bind('click touch', function (e) {
			startRippleEffect(this, e);
		});
	...
	$.fn.toast = function (options) {
		...
		var openToast = function openToast() {
			$('#' + defaults.id).css({ bottom: '0px', opacity: 1, zIndex: 1 });
		},
	...
	$.fn.slider = function (options) {
		...
		var getHorizontal = function getHorizontal(event) {
		...
		if (defaults.fill) {
			if (!defaults.disabled) {
				defaults.fill.css({ width: getHorizontal() + '%' });
			}
			if (defaults.hasDiscrete) {
				defaults.discrete.css({ marginLeft: getHorizontal() + '%' }).html(getHorizontal());
			}
		...
		$(this).on('mouseup touchend', function () {
			...
			self.css({ cursor: 'default' });
			if (defaults.hasDiscrete) {
				defaults.discrete.css({ transform: 'scale(0)' });
			}
		});
		...
		$(this).on('mousemove touchmove click', function (event) {
			if (defaults.startDrag) {
				...
				if (defaults.hasDiscrete && defaults.steps) {
					progressValue = Math.round(progressValue / 20) * 20;
				}
				if (progressValue < 1) {
					self.addClass('zero');
				} else {
				...
				if (defaults.fill && !defaults.disabled) {
					defaults.fill.css({ width: progressValue + '%' });
				}
				if (defaults.hasDiscrete) {
					defaults.discrete.css({ marginLeft: progressValue + '%', transform: 'scale(1)' }).html(progressValue);
				}
	...
	$.fn.chips = function (options) {
		...
		var openChip = function openChip(elem) {
			var chip = $(elem),
